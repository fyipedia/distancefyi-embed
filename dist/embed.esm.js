/* distancefyi-embed v1.0.0 | MIT | https://widget.distancefyi.com */
...
function haversine(lat1, lon1, lat2, lon2) {
  ...
  return {
    ...
    miles: Math.round(km * KM_TO_MILES * 10) / 10,
    nauticalMiles: Math.round(km / 1.852 * 10) / 10
  };
}
function estimateTravelTime(km) {
  return {
    drivingHours: Math.round(km / 80 * 10) / 10,
    flyingHours: Math.round(km / 800 * 10) / 10
  };
}
function formatDistance(km, useImperial = false) {
  if (useImperial) {
    const miles = km * KM_TO_MILES;
    return miles < 1 ? `${Math.round(miles * 5280)} ft` : `${miles.toLocaleString(void 0, { maximumFractionDigits: 1 })} mi`;
  }
  return km < 1 ? `${Math.round(km * 1e3)} m` : `${km.toLocaleString(void 0, { maximumFractionDigits: 1 })} km`;
}
...
// src/tools/distance-calc.ts
function renderDistanceCalc(container, config) {
  container.innerHTML = `
    <div class="geofyi-tool-header">Distance Calculator</div>
    <div class="geofyi-tool-body">
      <div class="geofyi-tool-row">
        <label class="geofyi-tool-label">From (lat, lon)</label>
        <input class="geofyi-search-input" id="geofyi-from" placeholder="40.7128, -74.0060" />
      </div>
      <div class="geofyi-tool-row">
        <label class="geofyi-tool-label">To (lat, lon)</label>
        <input class="geofyi-search-input" id="geofyi-to" placeholder="51.5074, -0.1278" />
      </div>
      <button class="geofyi-search-btn" id="geofyi-calc-btn">Calculate</button>
      <div id="geofyi-result" class="geofyi-tool-result"></div>
    </div>
    ${poweredByHTML(config)}
  `;
  const fromInput = container.querySelector("#geofyi-from");
  const toInput = container.querySelector("#geofyi-to");
  const calcBtn = container.querySelector("#geofyi-calc-btn");
  const resultDiv = container.querySelector("#geofyi-result");
  calcBtn.addEventListener("click", () => {
    const fromParts = fromInput.value.split(",").map((s) => parseFloat(s.trim()));
    const toParts = toInput.value.split(",").map((s) => parseFloat(s.trim()));
    if (fromParts.length !== 2 || toParts.length !== 2 || fromParts.some(isNaN) || toParts.some(isNaN)) {
      resultDiv.innerHTML = '<div class="geofyi-tool-error">Enter valid coordinates (lat, lon)</div>';
      return;
    }
    const [lat1, lon1] = fromParts;
    const [lat2, lon2] = toParts;
    const dist = haversine(lat1, lon1, lat2, lon2);
    const travel = estimateTravelTime(dist.km);
    const dir = bearingToCompass(bearing(lat1, lon1, lat2, lon2));
    resultDiv.innerHTML = `
      <div class="geofyi-kv-rows">
        <div class="geofyi-kv-row"><span class="geofyi-kv-label">Distance</span><span class="geofyi-kv-value geofyi-mono">${formatDistance(dist.km)}</span></div>
        <div class="geofyi-kv-row"><span class="geofyi-kv-label">Miles</span><span class="geofyi-kv-value geofyi-mono">${formatDistance(dist.km, true)}</span></div>
        <div class="geofyi-kv-row"><span class="geofyi-kv-label">Nautical Miles</span><span class="geofyi-kv-value geofyi-mono">${dist.nauticalMiles.toLocaleString()} nmi</span></div>
        <div class="geofyi-kv-row"><span class="geofyi-kv-label">Direction</span><span class="geofyi-kv-value">${dir}</span></div>
        <div class="geofyi-kv-row"><span class="geofyi-kv-label">Driving (est.)</span><span class="geofyi-kv-value">${formatTravelTime(travel.drivingHours)}</span></div>
        <div class="geofyi-kv-row"><span class="geofyi-kv-label">Flying (est.)</span><span class="geofyi-kv-value">${formatTravelTime(travel.flyingHours)}</span></div>
      </div>
    `;
  });
}

// src/inline/distance-badge.ts
function renderDistanceBadge(data, _config) {
  var _a, _b, _c, _d, _e, _f, _g, _h;
  const lat1 = Number((_b = (_a = data.lat1) != null ? _a : data.from_lat) != null ? _b : 0);
  const lon1 = Number((_d = (_c = data.lon1) != null ? _c : data.from_lon) != null ? _d : 0);
  const lat2 = Number((_f = (_e = data.lat2) != null ? _e : data.to_lat) != null ? _f : 0);
  const lon2 = Number((_h = (_g = data.lon2) != null ? _g : data.to_lon) != null ? _h : 0);
  if (!lat1 && !lon1 && !lat2 && !lon2) return `<span class="geofyi-badge">No coordinates</span>`;
  const dist = haversine(lat1, lon1, lat2, lon2);
  return `<span class="geofyi-badge">${formatDistance(dist.km)}</span>`;
}

// src/_entry_distancefyi.ts
async function processElement(el, config) {
  ...
  switch (type) {
    ...
    case "distance-calc": {
      ...
      renderDistanceCalc(root, config);
      break;
    }
    case "distance-badge": {
      const shadow = createShadow(el, config);
      ...
      root.innerHTML = renderDistanceBadge(data, config);
      break;
    }
    default:
      console.warn(`[distancefyi-embed] Unknown widget type: ${type}`);
  }
}
...
(function bootstrap() {
  const config = '{"site":"distancefyi","name":"DistanceFYI","domain":"distancefyi.com","accent":"#EA580C","attribute":"data-distancefyi","apiBase":"https://distancefyi.com/api/v1/","searchPath":"/search/","entityName":"Cities","entitySlug":"cities"}';
...
function makeWidgetElement(widgetType, initFn, domainAttrs) {
  ...
  return class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._syncDataAttrs();
      initFn(this, '{"site":"distancefyi","name":"DistanceFYI","domain":"distancefyi.com","accent":"#EA580C","attribute":"data-distancefyi","apiBase":"https://distancefyi.com/api/v1/","searchPath":"/search/","entityName":"Cities","entitySlug":"cities"}');
    }
    ...
    _syncDataAttrs() {
      const attrKey = '{"site":"distancefyi","name":"DistanceFYI","domain":"distancefyi.com","accent":"#EA580C","attribute":"data-distancefyi","apiBase":"https://distancefyi.com/api/v1/","searchPath":"/search/","entityName":"Cities","entitySlug":"cities"}'.attribute.replace("data-", "");
...
(function registerElements() {
  ...
  const site = '{"site":"distancefyi","name":"DistanceFYI","domain":"distancefyi.com","accent":"#EA580C","attribute":"data-distancefyi","apiBase":"https://distancefyi.com/api/v1/","searchPath":"/search/","entityName":"Cities","entitySlug":"cities"}'.site;
  const defs = [
    ...
    [`${site}-distance-calc`, renderDistanceCalc, ["lat1", "lon1", "lat2", "lon2"]]
  ];
